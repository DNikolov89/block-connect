          ...
          <div className={`transition-all duration-300 ${searchOpen ? 'w-64' : 'w-0'} overflow-hidden`}>
            <div className="relative">
              <input
                type="text"
                placeholder="Search..."
                className={`w-full rounded-full bg-secondary px-4 py-2 pr-8 focus:outline-none focus:ring-2 focus:ring-primary/30 ${searchOpen ? 'opacity-100' : 'opacity-0'} transition-opacity duration-300`}
              />
              {searchOpen && (
                <Button 
                  variant="ghost" 
